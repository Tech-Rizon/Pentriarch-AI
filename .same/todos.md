...
[x] ?? Executor allowlist enforces scanType -> runner mapping inside dockerManager (no raw command execution path).
[x] ?? Entitlement middleware blocks scans for non-allowed scan types and non-owned targets at the API layer.
...
[x] ?? New schema objects for projects, targets, target_verifications, scan_jobs, findings, and evidence are applied and verified in Supabase.
[x] ?? Target status (active/paused) is enforced in policy and entitlement checks.
...
[x] ? ChatKit console header shows plan and its tool menu is driven by entitlements.
[x] ? Target selector UI is required so users can only scan onboarded targets (no raw domain entry in chat).
...
[x] ?? Audit log table and server-side audit entries are implemented for scan start/completion/denial.
